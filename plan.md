...
- 新しい会話を始める際に、利用者が AI モデルとシステムプロンプトを指定でき、その設定が会話中は一貫して使用される。
...
- 選択可能な AI モデルは、モデル ID と画面表示名を持つサーバー側の許可リストとして定数に集約し、既定モデルもその中から定める。実装時点の Workers AI モデルカタログを確認し、会話向けで `messages` 入力に対応するモデルだけを登録する。
- システムプロンプトには既定値を用意し、未入力の場合は既定値を保存する。最大入力長、システムプロンプトの最大長、文脈に含める最大メッセージ数も定数に集約する。
- AI モデルとシステムプロンプトは会話作成後に変更できない。設定変更が必要な場合は新しい会話を作成し、会話途中で応答特性や文脈の前提が変わることを防ぐ。
...
| `ai_model` | TEXT NOT NULL | 会話作成時に選択した Workers AI モデル ID |
| `system_prompt` | TEXT NOT NULL | 会話作成時に入力、または既定値を適用したシステムプロンプト |
...
`ai_model` と `system_prompt` は個々のメッセージではなく会話の属性として保存する。これにより履歴を再表示した後も同じ設定で応答を継続でき、メッセージごとの重複保存を避けられる。モデルの許可リストはアプリケーション側で検証し、データベースの `CHECK` 制約にはモデル ID を列挙しない。モデルの追加・廃止のたびにスキーマ変更が必要になることを避けるためである。システムプロンプトは秘密情報の保存場所として扱わず、その旨を入力画面に表示する。

既存データが存在する環境へ機能追加する場合は、初期スキーマを書き換えず、追加マイグレーションで `ai_model` と `system_prompt` を追加する。既存行にはリリース時の既定値をバックフィルした後にアプリケーションを切り替える。新規構築時の `0001_initial.sql` には上記 2 列を最初から含める。
...
| メソッド・パス | 処理 | 主な応答 |
| --- | --- | --- |
| `GET /api/models` | 新規会話で選択可能なモデルと既定値を取得 | `{ models: [...], defaultModel: "...", defaultSystemPrompt: "...", maxSystemPromptLength: 2000 }` |
| `GET /api/conversations` | 会話一覧を更新日時の降順で取得（モデル ID を含む） | `{ conversations: [...] }` |
| `POST /api/conversations` | モデルとシステムプロンプトを指定して空の新規会話を作成 | `201` と `{ conversation }` |
...
`POST /api/conversations` の JSON は `{ "model": "<モデル ID>", "systemPrompt": "<プロンプト>" }` とする。`model` が許可リストにない場合は `400` を返し、クライアントから任意のモデル ID を Workers AI に渡せないようにする。`systemPrompt` は文字列だけを受け付け、前後の空白を除去して、空なら既定値、上限超過なら `400` とする。作成応答と会話詳細には `aiModel` と `systemPrompt` を含めるが、会話一覧では表示に必要な `aiModel` のみを返して不要な転送量を抑える。
...
4. D1 から会話に保存された `ai_model`、`system_prompt` と直近のメッセージを上限件数だけ取得し、メッセージを時系列順の `messages` 配列に戻す。
5. 保存済みの `system_prompt` を system メッセージとして先頭に置き、`env.AI.run(保存済みの ai_model, { messages })` を 1 回呼び出す。送信 API のリクエストからモデルやシステムプロンプトを受け取らない。
6. 応答形式をランタイムで検証して本文を取り出す。空の応答や想定外形式はエラーとして扱う。
7. assistant メッセージを保存し、会話の `updated_at` を更新して返す。

AI 呼び出し失敗時も利用者メッセージは履歴に残し、再送を可能にする。保存済みモデルが Workers AI で廃止・利用不能になった場合も自動的に別モデルへ切り替えず、新しい会話を作るよう案内する一般化したエラーを返す。エラー詳細はサーバーログにのみ記録し、クライアントには追跡用リクエスト ID と一般化した文言だけを返す。意図しないコスト増加を避けるため、自動リトライは実装しない。

## 7. 画面と操作

単一ページに以下を配置する。

- ヘッダーと「新しい会話」ボタン。ボタンから新規会話設定フォームを開く。
- 新規会話設定フォーム内の AI モデル選択欄、システムプロンプト入力欄、作成・キャンセルボタン。モデルは `GET /api/models` の応答から選択肢を作り、任意文字列は入力させない。システムプロンプト欄には既定値と最大文字数を示す。
- 履歴を開閉するボタンと、会話題名・更新日時を表示する履歴領域。
- 利用者と AI を視覚的に区別したメッセージ領域。
- 複数行入力欄、送信ボタン、処理中表示、スクリーンリーダー向けの状態通知。
- 選択中の会話を確認してから消す削除ボタン。

初期表示でモデル一覧と履歴を読み込み、最新会話を表示する。履歴がない場合は新規会話設定フォームを表示し、利用者が設定を確定するまでは空の会話を自動作成しない。会話表示には使用中のモデル名を併記し、保存済みのシステムプロンプトは会話詳細の読み取り専用欄で確認できるようにする。作成中・送信中は二重操作を防ぎ、成功時はメッセージと履歴を再描画し、失敗時は入力内容を保持して日本語のエラーを表示する。本文やシステムプロンプトは `innerHTML` に渡さず `textContent` または textarea の `value` で描画し、AI 応答や保存値を介した XSS を防止する。
...
5. **履歴 API を実装する**
   - モデル一覧、会話一覧、設定付き作成、詳細、削除の各処理を prepared statement で実装する。
   - モデルの許可リスト、システムプロンプトの既定値・長さ、ID、件数、並び順、存在しない会話、関連メッセージ削除をテストする。
6. **チャット API を実装する**
   - 入力検証、会話設定と文脈の取得、Workers AI 呼び出し、回答保存を実装する。
   - AI バインディングをテスト用に差し替え、保存したモデルとシステムプロンプトの使用、成功、AI エラー、空応答、入力上限をテストする。
7. **iOS 9 対応 UI を実装する**
   - セマンティック HTML、互換 CSS、ES5 JavaScript で、新規会話設定フォームを含む画面と API クライアントを作る。
...
- 許可されたモデルと任意のシステムプロンプトで会話を作成でき、未入力のプロンプトには既定値が保存される。
- 許可されていないモデル、文字列でないプロンプト、長すぎるプロンプトでは会話が作成されない。
- 会話を再表示してメッセージを送っても、作成時に保存したモデルとシステムプロンプトが Workers AI 呼び出しに使われる。
...
- iOS 9 Safari でログイン、履歴一覧、モデルとシステムプロンプトを指定した新規会話、送信、再表示、削除を操作する。
...
初回実装では、ユーザー登録、複数ユーザー、OAuth、ファイル添付、Markdown/HTML 描画、回答ストリーミング、会話途中でのモデル・システムプロンプト変更、会話検索、ページネーション、会話共有は対象外とする。必要になった場合は、Basic 認証から Cloudflare Access 等への移行、ユーザー ID を含むスキーマ移行、カーソルベースのページネーションの順で拡張する。
