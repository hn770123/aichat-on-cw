...
| **一般的な対話・日常的な質問応答** | `@cf/meta/llama-3.1-8b-instruct` | **【標準高性能】** 精度・速度・リソース効率のバランスに優れ、汎用チャットボットや要約処理の標準モデル。 |
...
## 5. Worker から呼び出す際の実践的なコツ

### 5.1 設定とバインディング (`wrangler.jsonc`)
...
- **標準・バランス**: 一般的な対話処理にはデファクト標準の `Llama 3.1 8B` を使用。
